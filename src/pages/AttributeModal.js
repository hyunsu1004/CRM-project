...
const AttributeModal = ({ onClose, onSubmit, existingAttributes = [] }) => {
  ...
  useEffect(() => {
    if (dataType === "select" || dataType === "multiSelect") {
      setSelectedOptions(["Series A", "Series B", "Series C", "Series D"]);
      setDefaultValue("Series A"); // 기본값 설정
    } else if (dataType === "integer" || dataType === "string") {
      setSelectedOptions([]);
      setDefaultValue("");
    } else {
      setSelectedOptions([]);
      setDefaultValue(""); // 다른 자료형의 경우 초기화
    }
  }, [dataType]);
