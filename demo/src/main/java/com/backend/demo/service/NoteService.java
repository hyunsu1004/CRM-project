...
public class NoteService {
    ...
    //딜 페이지 하나의 노트 삭제
    // 하나의 노트 삭제
    public void deleteNoteForDeal(int noteId) {
        DealNote dealNote = dealNoteRepository.findById(noteId)
    ...
    //딜 페이지 여러 노트 삭제
    public void deleteNotesForDeal(int dealId, List<Integer> noteIds) {
        List<DealNote> dealNotes = dealNoteRepository.findAllById(noteIds);

        for(DealNote dealNote : dealNotes){
            if(dealNote.getDeal().getId() == dealId){
                throw new IllegalArgumentException("해당 딜에 연결된 노트가 아닙니다.");
            }
        }
        dealNoteRepository.deleteAll(dealNotes);
    }

    //특정 DealId에 해당하는 모든 노트 가져오기.
    public List<DealNote> getNotesByDealId(int dealId) {
        List<DealNote> notes = dealNoteRepository.findByDeal_Id(dealId);
        if(notes.isEmpty()){
            System.out.println("노트가 발견되지 않았습니다. " + dealId);
        }else{
            System.out.println("노트가 발견되었습니다. " + dealId);
        }
        return notes;
    }
